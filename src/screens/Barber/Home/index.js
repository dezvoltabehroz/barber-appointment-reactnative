...
class BarberHome extends Component {
    ...
    _renderBookingItems = (item, index) => {
        ...
        return (
            <TouchableOpacity disabled={item.is_accepted == '1' ? false : true} onPress={() => onView(region, item.id, item.customer_id, item.booking_date, item.booking_time, item.booking_time_duration,item.customer_address)} style={{ backgroundColor: '#171717', marginHorizontal: '5%', paddingHorizontal: '1%', borderWidth: 1, borderColor: 'gray' }}>
                ...
                <Modal isVisible={this.props.user.modal}>
                    <View style={{ backgroundColor: '#171717', borderRadius: 10, }}>
                        ...
                        <View style={{ paddingTop: '5%', marginHorizontal: "25%",paddingBottom: '5%', }}>
                            <TouchableOpacity onPress={() => { this.props.authActions.healthAndSafety(false) }} style={{ backgroundColor: THEME.PRIMARY_COLOR, height: 50,  justifyContent: 'center' }}>
